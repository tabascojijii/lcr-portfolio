...
- 実行ログに以下を必須記録:
  - コンテナイメージdigest
  - 実行時Gitコミットハッシュ
- 監査レコードのハッシュ対象は「入力・出力・実行ログ本体・主要パラメータ」を必須とし、各SHA-256および集約ハッシュを保存する。
- パスはプロジェクトルート相対で統一し、絶対パス入力は fail-fast で拒否する。
...
- 監査不変条件として「相対パス強制（絶対パスはfail-fast）」と「ハッシュ対象完全化（入力・出力・実行ログ本体・主要パラメータ）」を実装制約として固定。
- `MainWindow` から業務判断ロジックを排除し、UseCase/Domainへ移管する。

5. 検証・監査証跡作成
- `pytest tests/` 実行。
- 再検証結果、ハッシュ、ログ、未達有無を監査可能形式で記録。
- 準拠性テスト（相対パス違反検知、ハッシュ網羅性、UI層の業務ロジック不在）を必須実行する。
...
- 準拠性テスト3点（相対パス違反検知、ハッシュ網羅性、UI層業務ロジック不在）が全件Pass。
