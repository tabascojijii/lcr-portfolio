...
- `docs/audit_report.md` の現時点判定（`AUDIT_PASS_PLAN`）を維持条件として扱い、PASSを理由に境界是正タスクを省略しない。
...
### 1.4 監査PASS時の非緩和原則（Audit Report整合）
- `docs/audit_report.md` で重大/軽微指摘が0件でも、RC-1〜RC-3由来の統制（P0/P0.1/P0.2）は削除・延期しない。
- 監査PASSは「計画の妥当性証明」であり「実装完了宣言」ではない。実装工程では4ゲート実測を必須化する。
...
5. PASS維持条件の固定
- `docs/audit_report.md` の PASS根拠（基準章1〜4適合）をチェックリスト化し、各PR/各監査で再確認する。
- PASS根拠と矛盾する差分（例: UI責務逆流、Port省略、監査項目欠落）は即時 `REJECT` とする。
...
- `audit_report` がPASSでも、P0未完了・4ゲート未計測・証跡欠落のいずれかがあれば進行停止する。
